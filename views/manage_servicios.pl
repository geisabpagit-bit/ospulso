...
# Seguridad: Validar si la matriz RBAC otorga permiso de Lectura en Servicios
unless (utils::permisos_utils::tiene_permiso_modulo($id_empresa, $role, 'servicios', 'R')) {
    render_acceso_denegado(
        q => $q, usuario => $usuario, role => $role,
        mensaje => 'No cuenta con facultades para acceder al módulo de Servicios de la Organización.',
        rol_requerido => 'Facultad de Lectura (R) en Servicios'
    );
    exit;
}
