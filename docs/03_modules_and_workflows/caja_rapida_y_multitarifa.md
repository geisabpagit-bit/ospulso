...
2. **Selector de Tarifa en la Tabla Modal**: Si un servicio posee múltiples tarifas comerciales activas para el tipo de paciente (ej. `ESTÁNDAR`, `URGENCIAS`, `DOMINGOS Y FESTIVOS`), la columna de precio renderiza un selector `<select id="selTarifaModal_${id}">` estilizado como una píldora médica (`rounded-pill fw-bold border-info-subtle bg-light text-primary shadow-sm ps-2.5 pe-4 py-1`). El atributo `pe-4` garantiza la zona de protección para la flecha chevron evitando cualquier solapamiento visual con el texto. Si solo cuenta con 1 tarifa, muestra la cifra estática.
