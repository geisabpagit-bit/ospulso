...
**Ámbito**: Módulo de Consultas Privadas, Gestión de Personal y Expedientes  
...
Esta arquitectura garantiza que el software pueda crecer hasta incluir decenas de especialidades médicas (Pediatría, Ginecología, Odontología, Cirugía, Nutriología, Cardiología, etc.) **sin duplicar código operativo** (Caja, Agenda, Expedientes, Firma Digital, PDF) y sin generar un monolito ingobernable.

---

## 🛡️ 2. Las 3 Reglas de Oro de Arquitectura SOAP Polimórfica
...
- La pestaña **Exploración Física / Objetivo (O)** evalúa el `id_espe_medico` cargado inmutablemente desde el perfil del médico (`usuarios.dat`):
  - **Si existe subformulario nativo (ej. Odontología / ID 100)**: Renderiza la herramienta específica (Odontograma interactivo sobre Canvas HTML5).
  - **Si no existe aún el subformulario nativo**: Renderiza automáticamente el **Módulo de Fallback Universal**, mostrando los Signos Vitales básicos y la leyenda del módulo dinámico.
...
## 🔗 4. Conexión con los Flujos de Caja y Recibos
Al concluir la nota SOAP (Paso 6 y 7):
1. **Cargos Directos**: Los servicios adicionales seleccionados se serializan en `caja_items_json`.
2. **Transferencia a Caja Rápida / Recibos**: Se registra la orden en `dat/estado_cuenta.dat` y se permite la emisión de recibos en [views/generar_recibo.pl](file:///c:/xampp/htdocs/ospulso/views/generar_recibo.pl) aplicando la matriz Multi-Tarifa (`ESTÁNDAR`, `MUNICIPIO`, `URGENCIAS`).
