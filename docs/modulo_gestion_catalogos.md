...
Para evitar que el c�digo del frontend se convierta en un 'Spaghetti Code' lleno de condiciones espec�ficas para cada tabla, la gesti�n de cat�logos emplea una arquitectura modular basada en Hooks (CatalogHooks).

Al crear o editar registros en cat�logos especiales (ej. medicos_<CLUE>.dat), el sistema invoca m�todos de un objeto de configuraci�n para personalizar la experiencia sin afectar el n�cleo del CRUD:
- preload: Carga datos for�neos antes de mostrar el modal (ej. cargar especialidades_<CLUE>.dat).
- onCalculateId: Intercepta la generaci�n de un nuevo ID para aplicar reglas de autoincremento.
- onRenderField: Transforma un campo de texto est�ndar en un componente din�mico (ej. convertir \ en un men� <select>).
