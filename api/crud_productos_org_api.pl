...
unless (utils::permisos_utils::tiene_permiso_modulo($id_empresa, $role, 'productos', $accion_req)) {
    print encode_json({ status => 'error', message => 'Permisos insuficientes para realizar esta acción en Productos.' });
    exit;
}
